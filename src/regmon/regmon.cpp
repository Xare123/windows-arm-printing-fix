...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	// The registered monitor name MUST match szMonitorName ("clawmon") in defs.cpp,
	// because EnumPorts reports each port's pMonitorName as szMonitorName and the
	// spooler looks the monitor up by that name when binding a printer to the port.
	// Optional argv[2] overrides the name (e.g. to deregister an old/mis-named monitor).
	minfo.pName = (argc > 2) ? argv[2] : (LPTSTR)_T("clawmon");
